...
public class GameScreen implements Screen, InputProcessor {
	...
	public void handleDog(float dt) {
        if (move_dog && attack_delay > dog_delay) {
            ...
            if (dog.getBody().getPosition().x > camera.position.x / 100f + IronCurtain.screenWidth / 200f && !dog.get_sound_played()) {
                barking.play(volume);
                dog.set_sound_played(true);
            }

            dog.update(dt);
            batch.draw(dog.getDog(), dog.getPosition().x, dog.getPosition().y, dog.getWidth(), dog.getHeight());
        }
    }

    void handleMines(float dt) {
